...
export default function ThreeDPage() {
  ...
  const threeDToolIds = threeDTools.map((t) => t.id);
  useEffect(() => {
    if (!threeDToolIds.includes(activeToolId)) {
      setActiveToolId("3d-generator");
    }
  ...
  const activeTool = threeDTools.find((t) => t.id === activeToolId) ?? threeDTools[0];
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 gap-1 px-1 pb-1">
        ...
        >
          {showToolsPanel ? (
            <AllToolsPanel initialTab="3d" onClose={() => setShowToolsPanel(false)} />
          ) : (
          <div key="controls" className="flex flex-col gap-4 px-4">
            ...
            <div
              ...
              style={{
                background: paletteColors["3d"].bg,
              }}
            >
              <button
                type="button"
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                <Cube weight="bold" size={20} className="shrink-0" />
                <span className="flex-1 text-left">Meshy v4</span>
